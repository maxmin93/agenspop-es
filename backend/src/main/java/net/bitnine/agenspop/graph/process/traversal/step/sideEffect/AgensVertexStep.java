...
import net.bitnine.agenspop.graph.process.util.CloseableIteratorUtils;
...
public class AgensVertexStep<E extends Element> extends VertexStep<E> implements HasContainerHolder {

    private final List<HasContainer> hasContainers = new ArrayList<>();

    public AgensVertexStep(final VertexStep<E> originalVertexStep) {
...
//                    if (graph.hasIndex(OperationType.READ, ElementType.EDGE, label, hasContainer.getKey())) {
                        return IteratorUtils.stream(
                                    ((AgensVertex) traverser.get())
                                    .edges(getDirection(), label, hasContainer.getKey(), hasContainer.getValue())
                                )
