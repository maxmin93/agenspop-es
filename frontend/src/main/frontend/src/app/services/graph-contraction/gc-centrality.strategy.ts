...
export class GcCentralityStrategy implements GcStrategy {
  ...
  constructor(cy:any, method:string){
    this.cy = cy;
    ...
    this.bins = this.make_bins(this.components, ranks);
    ...
    for( let bin of this.bins ){
      bin[this.topRank-1].forEach(e=>e.addClass('seed-semi'));
      bin[this.topRank].forEach(e=>e.addClass('seed'));
    }
  ...
  doExpansion(currRank:number){
    ...
    for( let i=0; i<this.components.length; i+=1 ){
      ...
      // remove higher links
      let links = this.cy.edges('.gclink').filter(e=>e.scratch('_rank')>currRank).remove();
      if( links.size() > 0 ){
        // for DEBUG
  ...
  make_bins(components:any[], ranks:Array<any[]>):Array<Array<any>> {
