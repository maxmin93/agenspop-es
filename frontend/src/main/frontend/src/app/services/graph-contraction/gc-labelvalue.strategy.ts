...
export class GcLabelValueStrategy implements GcStrategy {
  ...
  private groupsOfSeeds:any[];          // groups of seeds by value, ex) customer.country => { USA, France, Japan, ... }

  constructor(cy:any, label:string, key:string){
    this.cy = cy;
    this.label = label;
    this.key = key;

    // let labels = cy.nodes(':visible').filter(e=>e.data('label')==label && e.data('properties').hasOwnProperty(key)).toArray();
    // this.groups = _.groupBy(labels, e=>e.data('properties')[key]);
    // console.log('0) groupBy['+label+']['+key+']='+labels.length, this.groups);
    ...
    //    - label nodes with key 가 없는 component 제거 => singles 에 추가
    //    - "seeds" 에 label nodes 을 저장 => 나중에 topRank 부여
    this.components = this.split_components(this.elements, label, key);
    this.groupsOfSeeds = this.groupByLabelKey(this.seeds, key);
    // console.log('1) split_components='+this.components.length, this.components, this.groupsOfSeeds);
    ...
    this.bins = this.make_bins(this.components, ranks, this.groupsOfSeeds);
    ...
    for( let bin of this.bins ){
      ...
      if( bin[this.topRank+1] ) bin[this.topRank+1].forEach(e=>e.addClass('seed'));
      if( bin[this.topRank+2] ) bin[this.topRank+2].forEach(e=>e.addClass('seed'));
    }
  ...
  doContraction(currRank:number){
    ...
    for( let i=0; i<this.components.length; i+=1 ){
      ...
      if( !this.bins[i][currRank] || this.bins[i][currRank].size() == 0 ) continue;    // WARN: sometimes happen!
  ...
  // ==> array of component
  split_components(elements:any, label:string, key:string):any[]{
    // exclude solo elements
    let components = elements.components().filter(x=>x.size()>1);
    let componentOrder = 0;
    for( let i=0; i<components.length; i+=1 ){
      let labelNodesCnt = components[i].nodes().filter(x=>x.data('label')==label && x.data('properties').hasOwnProperty(key)).size();
    ...
    for( let i=0; i<componentOrder; i+=1 ){
      this.seeds[i] = components[i].nodes().filter(x=>x.data('label')==label && x.data('properties').hasOwnProperty(key));
    }

    return components;
  }

  groupByLabelKey(seeds:any[], key:string):any[] {
    let groups = [];
    for( let i=0; i<seeds.length; i+=1 ){
      let groupBy = _.groupBy( seeds[i].toArray(), e=>e.data('properties')[key]);
      groups.push( groupBy );
    }
    return groups;
  }
  ...
  make_bins(components:any[], ranks:Array<any[]>, groupsOfSeeds:any[]):Array<Array<any>> {
    ...
    for( let component of components ){
      ...
      bins[i] = new Array<any>(this.topRank+3);     // +1 for semi-seeds, +2 for seeds, +3 for seed of seeds
      ...
      if( this.seeds[i].size() == 1){       // 1개인 경우는 똑같음
        bins[i][this.topRank+2] = this.seeds[i];
        bins[i][this.topRank+2].scratch('_rank', this.topRank+2);
        this.seeds[i].style('label', this.seeds[i].first().data('properties')[this.key]);
      }
      else if( this.seeds[i].size() > 1 ){
        bins[i][this.topRank] = this.seeds[i];      // semi-seeds
        bins[i][this.topRank].scratch('_rank', this.topRank);
        bins[i][this.topRank+1] = this.cy.collection();

        // key의 value 가 One class 인 경우 ==> higher 노드 하나 생성
        if( Object.keys(groupsOfSeeds[i]).length == 1 ){
          // label nodes 가 많은 component 에는 temp seed 생성 => 보기 좋으라고 (데코)
          let boundingBox = this.seeds[i].boundingBox();
          let seed = this.create_temp_seed(i, this.topRank+2, this.label, this.seeds[i].first().scratch('_color')
          , {x:boundingBox.x1+2*boundingBox.w/3, y:boundingBox.y1+2*boundingBox.h/3});
          seed.data('properties')[this.key] = Object.keys(groupsOfSeeds[i])[0];     // set property with key
          seed.style('label', Object.keys(groupsOfSeeds[i])[0]);                    // style : caption
          bins[i][this.topRank+2] = seed;         // create seed-node
          components[i] = components[i].union(seed);
          this.seeds[i].forEach(e=>{              // create seed-links
            let link = this.create_temp_link(i, this.topRank+2, e, seed);
            components[i] = components[i].union(link);
          });
        }
        // key의 value 가 Multi class 인 경우 ==> higher 노드를 class 별로 생성
        else{
          let valueSeeds = this.cy.collection();
          for( let [val,arr] of Object.entries(groupsOfSeeds[i]) ){
            if( !arr || !(arr instanceof Array) ) continue;
            let nodes = this.cy.collection();
            arr.forEach(x=>{ nodes = nodes.union(x); });
            let boundingBox = nodes.boundingBox();
            let position = arr.length==1 ? {x:boundingBox.x1+60, y:boundingBox.y1+60} :
                            {x:boundingBox.x1+2*boundingBox.w/3, y:boundingBox.y1+2*boundingBox.h/3};
            let seed = this.create_temp_seed(i, this.topRank+1, this.label, nodes.first().scratch('_color'), position);
            seed.data('properties')[this.key] = val;                                // set property with key
            seed.style('label', val);                                               // style : caption
            bins[i][this.topRank+1] = bins[i][this.topRank+1].union(seed);          // create and add seed-node
            valueSeeds = valueSeeds.union(seed);
            components[i] = components[i].union(seed);
            arr.forEach(e=>{                      // create seed-links
              let link = this.create_temp_link(i, this.topRank+1, e, seed);
              components[i] = components[i].union(link);
            });
          }
          // create seed of seeds
          let boundingBox = valueSeeds.boundingBox();
          let position = {x:boundingBox.x1+2*boundingBox.w/3, y:boundingBox.y1+2*boundingBox.h/3};
          let seedOfSeeds = this.create_temp_seed(i, this.topRank+2, this.label, valueSeeds.first().scratch('_color'), position);
          seedOfSeeds.addClass('seed');
          seedOfSeeds.style('border-color', '#8000ff');
          components[i] = components[i].union(seedOfSeeds);
          valueSeeds.forEach(e=>{                      // create seed-links
            let link = this.create_temp_link(i, this.topRank+2, e, seedOfSeeds);
            components[i] = components[i].union(link);
          });
        }
